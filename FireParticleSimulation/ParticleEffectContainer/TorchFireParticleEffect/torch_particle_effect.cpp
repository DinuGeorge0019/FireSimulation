...
#include "torch_particle_effect.h"
#include "../particle.h"
...
TorchParticleEffect::TorchParticleEffect(GLuint particle_texture, glm::vec3 pos)
	: ParticleEffect(particle_texture, pos)
{
	this->max_particles = 300;
}

TorchParticleEffect::~TorchParticleEffect()
{

}

void TorchParticleEffect::create(GLuint program, Camera* camera)
{
...
void TorchParticleEffect::drawParticles(glm::mat4 projection_matrix, glm::mat4 view_matrix)
{
	...
	for (int i = 0; i<newparticles; i++){
		...
		this->particles_container[particleIndex].pos = glm::vec3(0, -10.0f, 0);

		this->particles_container[particleIndex].pos += glm::vec3(position.x, position.y + 5, position.z);
        this->particles_container[particleIndex].speed = glm::vec3(0.0f, 3.0f, 0.0f); // Fire propagates upwards
	}
